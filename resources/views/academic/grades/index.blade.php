...
@section('content')
    <div class="card">
        <div class="card-body">
            <table id="dg" title="Cursos" class="easyui-datagrid" style="width:100%;height:450px"
                url="{{ route('academic.grades.data') }}" toolbar="#toolbar" pagination="true" rownumbers="true"
        ...
        function destroyItem() {
            ...
            if (row) {
                $.messager.confirm('Confirmar', '¿Está seguro de eliminar este curso?', function (r) {
                    if (r) {
                        $.post("{{ url('academic/grades') }}/" + row.id, { _method: 'DELETE' }, function (result) {
                            if (result.success) {
                                $('#dg').datagrid('reload');
                            } else {
                                $.messager.show({
