...
@section('page-title', 'Administración de Bachilleratos')

@section('content')
    <div class="card">
        <div class="card-body">
            <table id="dg" title="Bachilleratos" class="easyui-datagrid" style="width:100%;height:450px"
                url="{{ route('academic.bachilleratos.data') }}" toolbar="#toolbar" pagination="true" rownumbers="true"
                fitColumns="true" singleSelect="true">
                <thead>
                    <tr>
                        <th field="id" width="50" sortable="true">ID</th>
                        <th field="name" width="200" sortable="true">Nombre del Bachillerato</th>
            ...
            <div id="toolbar">
                <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" plain="true"
                    onclick="newItem()">Nuevo Bachillerato</a>
...
@push('scripts')
    <script type="text/javascript">
        var url;
        function newItem() {
            $('#dlg').dialog('open').dialog('center').dialog('setTitle', 'Nuevo Bachillerato');
            $('#fm').form('clear');
            $('#fm').form('load', { is_active: 1 });
            url = "{{ route('academic.bachilleratos.store') }}";
        }
        function editItem() {
            var row = $('#dg').datagrid('getSelected');
            if (row) {
                $('#dlg').dialog('open').dialog('center').dialog('setTitle', 'Editar Bachillerato');
                $('#fm').form('load', row);
                url = "{{ url('academic/bachilleratos') }}/" + row.id;
        ...
        function destroyItem() {
            var row = $('#dg').datagrid('getSelected');
            if (row) {
                $.messager.confirm('Confirmar', '¿Está seguro de eliminar este bachillerato?', function (r) {
                    if (r) {
                        $.post("{{ url('academic/bachilleratos') }}/" + row.id, { _method: 'DELETE' }, function (result) {
                            if (result.success) {
                                $('#dg').datagrid('reload');
                            } else {
                                $.messager.show({
