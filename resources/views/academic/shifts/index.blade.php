...
@section('page-title', 'Administración de Turnos')

@section('content')
    <div class="card">
        <div class="card-body">
            <table id="dg" title="Turnos" class="easyui-datagrid" style="width:100%;height:450px"
                url="{{ route('academic.shifts.data') }}" toolbar="#toolbar" pagination="true" rownumbers="true"
                fitColumns="true" singleSelect="true">
                <thead>
                    <tr>
                        <th field="id" width="50" sortable="true">ID</th>
                        <th field="name" width="200" sortable="true">Nombre del Turno</th>
...
@push('scripts')
    <script type="text/javascript">
        var url;
        function newItem() {
            $('#dlg').dialog('open').dialog('center').dialog('setTitle', 'Nuevo Turno');
            $('#fm').form('clear');
            $('#fm').form('load', { is_active: 1 });
            url = "{{ route('academic.shifts.store') }}";
        }
        function editItem() {
            var row = $('#dg').datagrid('getSelected');
            if (row) {
                $('#dlg').dialog('open').dialog('center').dialog('setTitle', 'Editar Turno');
                $('#fm').form('load', row);
                url = "{{ url('academic/shifts') }}/" + row.id;
        ...
        function destroyItem() {
            var row = $('#dg').datagrid('getSelected');
            if (row) {
                $.messager.confirm('Confirmar', '¿Está seguro de eliminar este turno?', function (r) {
                    if (r) {
                        $.post("{{ url('academic/shifts') }}/" + row.id, { _method: 'DELETE' }, function (result) {
                            if (result.success) {
                                $('#dg').datagrid('reload');
                            } else {
                                $.messager.show({
