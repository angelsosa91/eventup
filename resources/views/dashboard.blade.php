                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h4 class="mb-0 {{ count($lowStockProducts) > 0 ? 'text-danger' : '' }}">
                                {{ count($lowStockProducts) }}</h4>
                        </div>
                        <i
                            class="bi bi-exclamation-triangle fs-2 {{ count($lowStockProducts) > 0 ? 'text-danger' : 'text-muted' }}"></i>
                    </div>
                </div>
            </div>
        </div>
                            ...
                            @foreach($salesByDay as $day)
                                <div class="col text-center">
                                    <div class="d-flex flex-column align-items-center">
                                        <div
                                            style="height: 150px; width: 100%; display: flex; align-items: flex-end; justify-content: center;">
                                            <div class="bg-primary"
                                                style="width: 60%; height: {{ ($day->total / $maxTotal) * 100 }}%; min-height: 5px; border-radius: 4px 4px 0 0;">
                                            </div>
                                        </div>
                                        <small class="fw-bold mt-2">{{ number_format($day->total, 0, ',', '.') }}</small>
                                        <small
                                            class="text-muted">{{ \Carbon\Carbon::parse($day->sale_date)->format('d/m') }}</small>
                                    </div>
                                </div>
                            @endforeach
                        </div>
