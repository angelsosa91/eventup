...
<div class="container-fluid">
    <div class="d-flex align-items-center mb-3 gap-3">
        ...
        <span class="badge bg-{{ $event->status === 'confirmed' ? 'success' : ($event->status === 'cancelled' ? 'danger' : 'secondary') }}">
            {{ $event->status_label }}
        </span>
    </div>
    ...
    <div class="easyui-tabs" style="width:100%;height:700px;">
        ...
        <div title="General" style="padding:20px;">
            <div class="row">
                <div class="col-md-4">
                    ...
                    <p><strong>Presupuesto Estimado:</strong> {{ number_format($event->estimated_budget, 0, ',', '.') }} Gs.</p>
                </div>
                <div class="col-md-8">
        ...
        <div title="Items" style="padding:10px;">
            <div id="tb-items" class="p-2 border-bottom mb-2">
                <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" onclick="newItemManual()">Nuevo Item</a>
                <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" onclick="openCatalogDialog()">Agregar del Catálogo</a>
                <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" onclick="editItem()">Editar</a>
                <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" onclick="removeItem()">Eliminar</a>
                
                <span class="ms-4 fw-bold text-primary">
                    Total Items: <span id="budget-total">{{ number_format($event->items->sum('total'), 0, ',', '.') }}</span> Gs.
                </span>
            </div>
            ...
            <table id="dg-items" class="easyui-datagrid" style="width:100%;height:580px"
                ...
                <thead>
                    <tr>
                        ...
                        <th data-options="field:'amount',width:150,align:'right',styler:function(){return 'font-weight:bold;'}">Monto (Gs.)</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ number_format($item->estimated_unit_price, 0, ',', '.') }}</td>
                        <td>{{ $item->notes }}</td>
                    </tr>
                    @endforeach
                </tbody>
        ...
        <div title="Mesas" style="padding:10px;">
            <div id="tb-tables" class="p-2 border-bottom mb-2">
                <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" onclick="addTable()">Agregar Mesa</a>
            </div>
            <table id="dg-tables" class="easyui-datagrid" style="width:100%;height:580px"
                   data-options="singleSelect:true,fitColumns:true,rownumbers:true,toolbar:'#tb-tables'">
                <thead>
                    <tr>
...
<div id="dlg-item-manual" class="easyui-dialog" style="width:450px" 
    ...
    <form id="fm-item-manual" method="post" novalidate style="padding:20px">
        <div class="mb-3">
            ...
            <input class="easyui-textbox" name="description" id="manual-description" style="width:100%" data-options="required:true,prompt:'Ej: Alquiler de Luces'">
        </div>
        <div class="mb-3">
            <label class="form-label">Monto (Gs.):</label>
            <input class="easyui-numberbox" name="estimated_unit_price" id="manual-price" style="width:100%" data-options="required:true,min:0,groupSeparator:'.'">
        </div>
        <div class="mb-3">
...
<div id="item-manual-buttons">
    <a href="javascript:void(0)" class="easyui-linkbutton c6" iconCls="icon-ok" onclick="saveItemManual()" style="width:90px">Guardar</a>
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#dlg-item-manual').dialog('close')" style="width:90px">Cancelar</a>
</div>
...
<div id="catalog-buttons">
    <a href="javascript:void(0)" class="easyui-linkbutton c6" iconCls="icon-ok" onclick="addSelectedItems()" style="width:120px">Agregar</a>
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#dlg-catalog').dialog('close')" style="width:90px">Cerrar</a>
</div>
...
<script>
    ...
    function editItem() {
        ...
        var price = row.amount.replace(/\./g, '');
        $('#manual-description').textbox('setValue', row.description);
        $('#manual-price').numberbox('setValue', price);
