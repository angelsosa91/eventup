...
@section('title', 'Configuración de la Entidad')
@section('page-title', 'Configuración de la Entidad')

@section('content')
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0">Datos de la Entidad</h5>
        </div>
        <div class="card-body">
            @if(session('success'))
                ...
                <div class="row">
                    ...
                    <div class="col-md-8">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="slogan" class="form-control @error('slogan') is-invalid @enderror" rows="2"
                                placeholder="Frase o lema de la empresa">{{ old('slogan', $settings->slogan) }}</textarea>
                            @error('slogan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="currency" class="form-select @error('currency') is-invalid @enderror" required>
                            <option value="PYG" {{ old('currency', $settings->currency) == 'PYG' ? 'selected' : '' }}>
                                Guaraníes (PYG)</option>
                            <option value="USD" {{ old('currency', $settings->currency) == 'USD' ? 'selected' : '' }}>Dólares
                                (USD)</option>
                            <option value="EUR" {{ old('currency', $settings->currency) == 'EUR' ? 'selected' : '' }}>Euros
                                (EUR)</option>
                            <option value="ARS" {{ old('currency', $settings->currency) == 'ARS' ? 'selected' : '' }}>Pesos
                                Argentinos (ARS)</option>
                            <option value="BRL" {{ old('currency', $settings->currency) == 'BRL' ? 'selected' : '' }}>Reales
                                (BRL)</option>
                        </select>
                        @error('currency')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="decimal_places" class="form-select @error('decimal_places') is-invalid @enderror"
                            required>
                            <option value="0" {{ old('decimal_places', $settings->decimal_places) == 0 ? 'selected' : '' }}>0
                                decimales</option>
                            <option value="2" {{ old('decimal_places', $settings->decimal_places) == 2 ? 'selected' : '' }}>2
                                decimales</option>
                            <option value="3" {{ old('decimal_places', $settings->decimal_places) == 3 ? 'selected' : '' }}>3
                                decimales</option>
                            <option value="4" {{ old('decimal_places', $settings->decimal_places) == 4 ? 'selected' : '' }}>4
                                decimales</option>
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="timezone" class="form-select @error('timezone') is-invalid @enderror" required>
                            ...
                            <option value="America/Buenos_Aires" {{ old('timezone', $settings->timezone) == 'America/Buenos_Aires' ? 'selected' : '' }}>América/Buenos Aires
                                (Argentina)</option>
                    ...
                    <div class="col-md-6 mb-3">
                        <div class="form-check mt-4">
                            ...
                            <input type="checkbox" name="invoice_requires_tax_id" value="1"
                                ...
                                id="invoice_requires_tax_id" {{ old('invoice_requires_tax_id', $settings->invoice_requires_tax_id) ? 'checked' : '' }}>
